...
- **`PlaygroundOverlay.jsx`/`.css` — removed.** Was a layout-mockup sandbox (drag/resize the page's real `<img>` elements, draw pen strokes/colored section boxes/sticky notes, `createPortal`-rendered so it never touched the page's own markup/CSS, state in `localStorage` per page) built to work out the hero's card sizing and the Partner's Attire white-frame markup. It was meant to stay a local dev-only tool but had no environment gating, so the floating "✎ Edit layout" button shipped to the live production page along with everything else in the "push live" round — caught after the fact and pulled out entirely (component files deleted, import + render removed from `Chatorey.jsx`), then redeployed. If a layout-mockup tool like this is wanted again, it needs to be gated (e.g. a build-time `import.meta.env.DEV` check, or a query-param flag) so it can never ship to production again.
