...
5. **Urgent, confirmed live:** `public/videos/` is gitignored, so none of those files exist in the repo the Actions build checks out from. Every video on the live site 404s right now (confirmed: `ashesharorastudios.com/videos/tib-hero.mp4` returns 404). This affects TIB's hero video and PCC's panel/vision-film/FRE8 videos at minimum. Swap for hosted embeds (YouTube/Vimeo unlisted) per the existing asset note, or commit the files directly if they're small enough. Either way this needs to happen before the site is actually shown to anyone, not just before some future "launch" milestone, since it's live now.
